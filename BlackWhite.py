...
for i in range(qty):  # show all dots one after another
    clock.reset(0.00)
    frame_count = 0
    fixation = visual.GratingStim(win=mywin, mask="none", size=20, pos=[0,0], sf=0, color=[-1,-1,-1], colorSpace='rgb')
    inverse_fixation = visual.GratingStim(win=mywin, mask="none", size=20, pos=[0,0], sf=0, color=[1,1,1], colorSpace='rgb')
